...
class EmployeeForm(forms.ModelForm):
    class Meta:
        model = Employee
        fields = ['first_name', 'second_name', 'surname', 'rank', 'degree', 'post', 'rate']
        widgets = {
...
class WorkloadDepartmentForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['workload', 'subgroups', 'hours']
        labels = {
            'workload': 'Нагрузка (предмет, группа, семестр)',
            'subgroups': 'Подгруппа',
            'hours': 'Часы на департамент',
        }
        widgets = {
            'workload': forms.Select(attrs={'class': 'form-select'}),
            'subgroups': forms.Select(attrs={'class': 'form-select'}),
    ...
    def clean(self):
            ...
            if workload and subgroups and hours and employee:
                ...
                all_teacher_hours = WorkloadTeacher.objects.filter(
                    employees=employee
                ).aggregate(Sum('hours'))['hours__sum'] or 0
